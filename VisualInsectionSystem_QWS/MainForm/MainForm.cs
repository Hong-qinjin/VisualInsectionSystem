...
using VisualInsectionSystem.Camera;
...
namespace VisualInsectionSystem
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            InitializeComponent();           
            InitializePLC();
            _cameraController = new HKCameraController();
        ...
        private void InitializePLC()
        {
            try
            {
                ...
                _plcComm = new PLCCommunicator(CpuType.S71200, "192.168.0.1", 0, 1);
                _plcComm.CameraReadyTriggered += async () => await ExecuteProcedureAsync(comboBox1.Text);                
        ...
        //private async void buttonExecuteOnce_Click(object sender, EventArgs e) => await ExecuteProcedureAsync(comboBox1.Text);
        ...
        public async Task<VMResult> ExecuteProcedureAsync(string procedureName)
        {
            ...
            {
                // 1. 相机拍照
                var capture = await _cameraController.CaptureOneFrameAsync(1000);
                if (!capture.Success) { LogHelper.Error($"拍照失败: {capture.ErrorMessage}"); return result; }
                tempImagePath = capture.ImageFilePath;

                // 2. 执行VM流程
                var vmProc = (VmProcedure)VmSolution.Instance[procedureName];
                if (vmProc == null) { LogHelper.Debug($"流程不存在: {procedureName}"); return result; }
                vmProc.Run();

                // 3. 解析结果                
                //string strResult = vmProcess.ModuResult.GetOutputString("out").astStringVal[0].strValue;
                string raw = vmProc.ModuResult.GetOutputString("out").astStringVal?[0].strValue;
                result = VMResult.Parse(raw);
                result.ExecuteTime = DateTime.Now;

                // 4. 发布事件
                OnInspectionCompleted(new InspectionCompletedEventArgs(result, tempImagePath));
                LogHelper.Info($"流程执行完成: {result.DetectResult}");
            }
            catch (Exception ex) { LogHelper.Error("执行流程异常", ex); LogHelper.Error($"执行异常: {ex.Message}"); }
            return result;
        }
        #endregion
        ...
        protected virtual void OnInspectionCompleted(InspectionCompletedEventArgs e) => InspectionCompleted?.Invoke(this, e);

        // 检测完成事件参数
        public class InspectionCompletedEventArgs : EventArgs
        {
            public VMResult Result { get; }
            public string TempImagePath { get; }
            public InspectionCompletedEventArgs(VMResult r, string path) 
            { 
                Result = r; 
                TempImagePath = path; 
            }
