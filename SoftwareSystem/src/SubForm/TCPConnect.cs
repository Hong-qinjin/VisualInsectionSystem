...
namespace VisualInsectionSystem.SubForms
{
    ...
    {
        private MainForm mainForm;                 //主界面应用update_1117

        // 网络通信相关变量
        private TcpListener tcpServer;          // TCP服务�?
        private TcpClient tcpClient;            // TCP客户�?
        ...
        private string currentProtocol = "";    // 当前选择的协议类�?
        private Dictionary<string, TcpClient> connectedClients = new Dictionary<string, TcpClient>();   // 存储已连接的客户�?

        private bool isHexSend = false;     // 是否16进制发�?
        ...
        {
            //currentProtocol = "TCP服务�?;
            string protocol = comboBox1.SelectedItem.ToString();
            bool isTcpServer = protocol == "TCP服务�?";
            //TCP服务端模式下,不需要目标IP和端�?
            ...
            if (isTcpServer)
            {
                button1.Text = isListening ? "停止监听" : "开始监�?";
            }
            else
            {
        ...
        {
            // 初始化协议类型下拉框
            comboBox1.Items.AddRange(new string[] { "TCP服务�?", "TCP客户�?", "UDP服务�?", "UDP客户�?"});
        ...
        {
            currentProtocol = comboBox1.SelectedItem.ToString();    // 获取当前选择的协议类�?

            if (currentProtocol == "TCP服务�?")
            {
                if (!isListening)
                {
                    StartTcpServer();
        ...
        private void StartTcpServer()
        {
            try
            {
                ...
                {
                    MessageBox.Show("端口号无�?");
                    return;
                }

                //IPAddress localAddr = IPAddress.Any;    // 监听所有网络接�?
                IPAddress localAddr = IPAddress.Parse(comboBox2.Text);  // 本地IP地址
                port = int.Parse(textBox1.Text);    // 本地端口�?
                tcpServer = new TcpListener(localAddr, port);  // 创建TCP服务�?

                tcpServer.Start();
                isListening = true;
                button1.Text = "停止监听";
                AddMessage($"TCP服务端已启动，监�?{localAddr}:{port}，等待连�?..");
            ...
            {
                MessageBox.Show($"启动TCP服务端失�? {ex.Message}");
            }
        }
        //停止客户�?add:当外部客户端断开�?
        private void StopTcpServer()
        {
            ...
            {
                MessageBox.Show($"停止TCP服务端错�? {ex.Message}");
            }
        }
        //监听client连接,add 持续监听
        ...
        private void ReceiveData()
        {
            try
            {
                ...
                while (isConnected && tcpClient != null && tcpClient.Connected)
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        Invoke(new Action(() =>
                        {
                            AddMessage($"�?{clientInfo} 收到: {GetDisplayText(data)}");
                        }));
                    ...
                    else if (bytesRead == 0)
                    {
                        ...
                        Invoke(new Action(() => AddMessage($"客户�?{clientInfo} 已断开连接!")));
                        break;
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    string clientInfo = GetClientInfo(tcpClient);
                    Invoke(new Action(() => AddMessage($"�?{clientInfo} 通信错误: {ex.Message}")));
                }
            }
            finally
            {
        ...
        private void SendData()
        {
            ...
            {
                MessageBox.Show($"发送失�? {ex.Message}");
            }
        }
        ...
        private void CleanupClientConnection(TcpClient client)
        {
            ...
            {
                Invoke(new Action(() => AddMessage($"清理客户端连接错�? {ex.Message}")));
            }
        }
                ...
                Invoke(new Action(() =>
                {
                    lock (connectedClients)
                    {
                        if (connectedClients.Count > 0)
                        {
                            ...
                            if (tcpClient != null && tcpClient.Connected)
                            {
                                stream = tcpClient.GetStream();     //client断开，异�?
                                isConnected = true;
        ...
        private void Button3_Click(object sender, EventArgs e)
        {
            if (currentProtocol != "TCP服务�?")
            {
                MessageBox.Show("仅TCP服务端模式下可发送数�?");
                return;
            }
            if (!isListening)
            {
                MessageBox.Show("请先启动TCP服务�?");
                return;
            }
            if (!isConnected || tcpClient == null || !tcpClient.Connected)
            {
                MessageBox.Show("请先建立连接,无法发送数�?");
                return;
            }
        ...
        private void AddMessage(string message)
        {
            ...
            listBox1.TopIndex = listBox1.Items.Count - 1; // 滚动到最新消�?
        }
        ...
        private void SaveReceivedMessages(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("没有消息可保�?");
                return;
            }

            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.Filter = "文本文件 (*.txt)|*.txt|所有文�?(*.*)|*.*";
            saveFileDialog.Title = "保存接收的消�?";
