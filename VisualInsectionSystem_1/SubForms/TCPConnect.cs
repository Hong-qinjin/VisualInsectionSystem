        ...
        private async Task StartServerAsync()
        {
            ...
            try
            {
                ...
                if (_currentProtocol == "TCP服务端")
                {
                    ...
                    LogHelper.Info($"TCP服务端已启动 {localAddr}:{localPort}");
                    ...
                    //};
                    //_listenThread.Start();
                    await Task.Run(() => ListenForTcpClients(_cts.Token));
                }
        ...
        private async Task ConnectToServerAsync()
        {
            ...
            try
            {
                ...
                else if (_currentProtocol == "UDP客户端")
                {
                    ...
                    LogHelper.Info($"UDP客户端已连接到 {remoteIP}:{remotePort}");
                ...
                try
                {
                    if (_tcpServer.Pending()) // 检查是否有挂起的连接
                    {
                        ...
                        Invoke(new Action(() =>
                        {
                            ...
                            LogHelper.Info($"客户端连接: {client.Client.RemoteEndPoint}");
                            _isConnected = true;
