...
namespace VisualInsectionSystem
{
    public partial class DebugForm : Form
    {
        ...
        private readonly Timer _loadSolutionIndicateTimer;
        private string cultureName = "zh-CN";
        private readonly string logPath = Application.StartupPath + "/Logs/Message";
        ...
        {
            InitializeComponent();
            renderControl = new DbgRenderControl { Dock = DockStyle.Fill };
            mainViewControl = new DebugFormControl { Dock = DockStyle.Fill };

            _loadSolutionIndicateTimer = new Timer { Interval = 300 };//设置定时器
            _loadSolutionIndicateTimer.Tick += LoadSolutionIndicateTimer_Tick;   //指示方案是否加载
        ...
        private void DebugForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            VmSolution.OnProcessStatusStartEvent -= VmSolution_OnProcessStatusStartEvent;
            VmSolution.OnProcessStatusStopEvent -= VmSolution_OnProcessStatusStopEvent;
            _loadSolutionIndicateTimer.Stop();

            if (!string.IsNullOrEmpty(currentSolutionPath) && SolutionIsLoaded)
            {
                //if (MessageBox.Show("Save solution or not? ", "Information", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                if (MessageBox.Show("是否保存当前方案？", "确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        VmSolution.Save();
        ...
        private void VmSolution_OnProcessStatusStartEvent(ImvsSdkDefine.IMVS_STATUS_PROCESS_START_CONTINUOUSLY_INFO statusInfo)
        {
            ...
            //}
            this.InvokeIfRequired(() => UpdateStartStatusUI(statusInfo));
        ...
        private void VmSolution_OnProcessStatusStopEvent(ImvsSdkDefine.IMVS_STATUS_PROCESS_STOP_INFO statusInfo)
        {
            ...
            //}
            this.InvokeIfRequired(() => UpdateStopStatusUI(statusInfo));
        }
        ...
        private void UpdateStartStatusUI(ImvsSdkDefine.IMVS_STATUS_PROCESS_START_CONTINUOUSLY_INFO statusInfo)
        {
            if (statusInfo.nStatus == 0)
            {
                ...
                buttonContiRun.Text = cultureName == "zh-CN"
                ...
                AppendLog("开始连续运行！Start continuous run!");
            }
        }
        ...
        private void AppendLog(string message)
        {
            ...
            try
            {
                if (!Directory.Exists("./Logs"))
                {
                    ...
                    File.AppendAllText("./log/Demo.log", $"{timeStamp}:{message}\n");
                }
        ...
        {
            string timeStamp = DateTime.Now.ToString("yy-MM-dd HH:mm:ss-fff");
            listBoxResult.InvokeIfRequired(() =>
            {
        ...
        private void UpdateLableState(string result)
        {
            try
            {
                ...
                bool isOK = strArray[0] == "1";
                labelResultState.InvokeIfRequired(()=>
                {
                    labelResultState.BackColor = isOK ? Color.FromArgb(0, 192, 0) : Color.Red;
                    labelResultState.Text = isOK ? "OK" : "NG";
                });
            }
            catch (Exception ex)
            {
        ...
        private void buttonSelectSolu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DialogResult.OK == oFresult)
                {
                    ...
                    _loadSolutionIndicateTimer.Enabled = true;
        ...
        {
            _loadSolutionIndicateTimer.Enabled = false;
        ...
        /// </summary>
        /// <param name="lst"></param>
        public void RegisterProcedureWorkEndCallback(List<VmProcedure> list)
        {
            try
            {
                foreach (var vmProcedure in list)
                {
        ...
        {
            //cultureName = cultureName == "zh-CN" ? "en-US" : "zh-CN";
