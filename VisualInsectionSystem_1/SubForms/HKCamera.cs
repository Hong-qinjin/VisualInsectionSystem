...
namespace VisualInsectionSystem.SubForms
{
    public partial class HKCamera : Form, IDisposable
    {
        ...
        {
            LogHelper.Info("尝试打开相机...");            

            if (true)
            { LogHelper.Info("相机打开成功"); }
            else
            { 
                LogHelper.Error("相机打开失败");
            }
        ...
        {
            string errorMsg=$"{csMessage} 错误码: {nErrorNum:X}";
            LogHelper.Error(errorMsg);
            MessageBox.Show(errorMsg, "相机错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
