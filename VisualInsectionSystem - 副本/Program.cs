    ...
    internal static class Program
    {
        ...
        private static void HandleException(Exception ex, string exceptionType)
        {
            try
            {
                ...
                // 显示友好错误提示
                DialogResult dialogResult = MessageBox.Show("发生错误，请检查日志文件。", "系统错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch
            {
                // 如果连错误日志都写不成功，就直接弹出消息框
                MessageBox.Show("系统严重错误，无法处理，请重新启动应用", "系统错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
